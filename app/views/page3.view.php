...
<script type="text/javascript">
    ...
    $('#ssdependent_form').submit(function (e) {
        e.preventDefault();
        var value = $("#ss_num").val();
        let form = $(this).serializeArray();

        $.ajax({
            url: "https://tran97.myweb.cs.uwindsor.ca/public/DependentController/getDependentsByEmployeeSSN",
            method: "get",
            data: form,
            dataType: "json"
        })
            .done((data) => {
                if (data) {
                    ...
                    if (data !== null && data.length > 0) {
                        list.innerHTML = "<h4 class=\"text-center\">Dependent List</h4>";
                        ...
                        data.forEach((item) => {
                            ...
                            list.querySelector('.list-group').innerHTML += `<li class="list-group-item">
                                <div class="d-flex justify-content-around">
                                    <div>Dependent Name: ${item.Dependent_name}</div>
                                    <div>Dependent Sex: ${item.Sex == 'M' ? 'Male' : 'Female'}</div>
                                </div>
                            </li>`;
                        });
                    }
